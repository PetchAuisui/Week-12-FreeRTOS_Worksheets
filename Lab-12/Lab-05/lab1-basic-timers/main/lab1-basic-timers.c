...
// ===== Experiment 3: Extra timer load (ONLY THIS EXPERIMENT) =====
#define EXTRA_TIMER_COUNT 10
static TimerHandle_t xExtraTimers[EXTRA_TIMER_COUNT];
...
// ===== Experiment 3 callback (ONLY THIS EXPERIMENT) =====
static void extra_callback(TimerHandle_t xTimer) {
    // แค่ log ความถี่ของแต่ละ extra timer (ไม่ block)
    uint32_t id = (uint32_t)(uintptr_t) pvTimerGetTimerID(xTimer);
    ESP_LOGD(TAG, "🧩 ExtraTimer[%lu] tick", (unsigned long)id);
}
// ========================================================
...
void app_main(void) {
    ...
    if (xBlinkTimer && xHeartbeatTimer && xStatusTimer && xOneShotTimer) {
        ...
        // ===== Experiment 3: create & start extra auto-reload timers (ONLY THIS EXPERIMENT) =====
        ESP_LOGI(TAG, "Creating %d extra auto-reload timers...", EXTRA_TIMER_COUNT);
        for (int i = 0; i < EXTRA_TIMER_COUNT; i++) {
            uint32_t period_ms = 100 + i * 50; // 100, 150, 200, ... 550 ms
            xExtraTimers[i] = xTimerCreate("ExtraTimer",
            ...
            if (xExtraTimers[i]) {
                if (xTimerStart(xExtraTimers[i], 0) != pdPASS) {
                    ESP_LOGW(TAG, "Start Extra[%d] FAILED", i);
                }
            } else {
                ESP_LOGW(TAG, "Create Extra[%d] FAILED", i);
            }
        }
        // =========================================================================================
